...
data_folder = "BCC 02-22 NDVI FImputed Cubic Spline s=1"
real_ndvi_folder = "BCC 02-22 NDVI FImputed Cubic Spline s=1"
error_maps_folder = "02-22 NDVI Error Maps"
grib_file = "ERA5.grib"
...
# Function to Extract Dates from Filenames (New Format "0222ndvi_YYYYMMDD_imputed_spline3.tif")
def extract_date_from_filename(filename):
    match = re.search(r'0222ndvi_(\d{8})\_imputed_spline3.tif$', filename)  # Adjusted for new format
    if match:
...
def load_ndvi_for_date(date, primary_folder, fallback_folder):
    ...
    """
    filename = f"0222ndvi_{date.strftime('%Y%m%d')}_imputed_spline3.tif"
...
# Save to CSV
train_csv_path = "training_data_0222.csv"
df_train.to_csv(train_csv_path, index=False)
print(f"✅ Training data saved to training_data_0222.csv")
...
#    "min_samples_split": [2, 4, 6, 8, 10]
# }
# ✅ Best Hyperparameters: {'max_depth': 10, 'max_features': 'sqrt', 'min_samples_leaf': 10, 'min_samples_split': 2, 'n_estimators': 50}
...
for pred_date in prediction_dates:
    ...
    if pred_date in special_dates:
        ...
        actual_filename = os.path.join(real_ndvi_folder, f"0222ndvi_{pred_date.strftime('%Y%m%d')}_imputed_spline3.tif")
    ...
    # Ensure future_features has the same shape as X_train (6363 features)
    expected_feature_count = 6219
    ...
    X_test = X_test[:, :6219]  # Trim excess features if needed
...
for i, pred in enumerate(predictions):
    ...
    if prediction_dates[i] in special_dates:
        ...
        actual_filename = os.path.join(real_ndvi_folder, f"0222ndvi_{prediction_dates[i].strftime('%Y%m%d')}_imputed_spline3.tif")
