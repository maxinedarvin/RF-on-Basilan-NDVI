...
data_folder = "BCC 04-03 NDVI FImputed Cubic Spline s=1"
real_ndvi_folder = "BCC 04-03 NDVI FImputed Cubic Spline s=1"
error_maps_folder = "04-03 NDVI Error Maps"
grib_file = "ERA5.grib"
...
# Function to Extract Dates from Filenames (New Format "0403ndvi_YYYYMMDD_imputed_spline3.tif")
def extract_date_from_filename(filename):
    match = re.search(r'0403ndvi_(\d{8})\_imputed_spline3.tif$', filename)  # Adjusted for new format
    if match:
...
def load_ndvi_for_date(date, primary_folder, fallback_folder):
    ...
    """
    filename = f"0403ndvi_{date.strftime('%Y%m%d')}_imputed_spline3.tif"
...
# Save to CSV
train_csv_path = "training_data_0403.csv"
...
plt.barh(top_10_features["Feature"], top_10_features["Importance"], color='purple')
plt.xlabel("Feature Importance")
plt.ylabel("Feature")
plt.title("Feature Importance Plot for Brgy. Boheibu")
...
for pred_date in prediction_dates:
    ...
    if pred_date in special_dates:
        ...
        actual_filename = os.path.join(real_ndvi_folder, f"0403ndvi_{pred_date.strftime('%Y%m%d')}_imputed_spline3.tif")
    ...
    # Load lagged NDVI from data_folder
    ...
    # Ensure future_features has the same shape as X_train (6357 features) # but 3684 expected features in 0403? +6 for rolling avg
    expected_feature_count = 3387 
    ...
    X_test = X_test[:, :3387]  # Trim excess features if needed
...
for i, pred in enumerate(predictions):
    ...
    if prediction_dates[i] in special_dates:
        ...
        actual_filename = os.path.join(real_ndvi_folder, f"0403ndvi_{prediction_dates[i].strftime('%Y%m%d')}_imputed_spline3.tif")
